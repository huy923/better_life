package com.health.heartrate.phone.sensor
 ...
 *
 * Algorithm:
 *  1. Open camera in torch mode to illuminate fingertip
 *  2. Capture frames and sample average pixel intensity
 *  3. Detect peaks in the intensity signal → inter-beat interval → BPM
 *
 * Accuracy: ±5 BPM at rest. Not a medical device.
 */
class CameraHeartRateSensor(private val context: Context) {
    ...
    private fun extractRedChannelIntensity(image: android.media.Image): Double {
        ...
        buffer.get(data)
        // YUV Y-plane approximates red intensity when flash illuminates red-saturated fingertip
        var sum = 0L
    ...
    companion object {
        ...
        private const val MIN_PEAK_GAP_MS          = 300L   // max 200 BPM
